...
function fillProducts() {
    $.ajax({
            ...
            for (var i = 0; i < items.length; i++) {
                if (items[i].forSale == true) {
                    $("#models").append(
                        ...
                        '<button type="button" id="btn-buy" class="btn btn-primary btn-modal" data-dismiss="modal" onclick="thanks(' + items[i].id + ')">Buy now</button>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
        ...
        error: function (responseJSON) {
            $('#successpst').text("There was an error, please try again later");
        }
    })
            ...
            for (var i = 0; i < items.length; i++) {
                if (items[i].forSale == true) {
                    $("#models").append(
                        ...
                        '<div id="buttons-modal">' +
                        '<button type="button" id="btn-cancel" class="btn btn-outline-success btn-modal" data-dismiss="modal">Cancel</button>' +
                        '<button type="button" id="btn-buy" class="btn btn-primary btn-modal" data-dismiss="modal" onclick="thanks(' + items[i] + ')">Buy now</button>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
